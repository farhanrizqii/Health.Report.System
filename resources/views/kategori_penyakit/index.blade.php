            ...
            <div class="data-card">
                <div class="text-gray-900 dark:text-gray-100">
                    ...
                    <div class="action-buttons-container"> 
                        <a href="{{ route('kategori-penyakit.create') }}" class="btn-tambah">
                            <i class="fas fa-plus"></i>
                            <span>Tambah Kategori Baru</span>
                        </a>
                        <a href="{{ route('kategori-penyakit.export') }}" class="btn-ekspor">
                    ...
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('kategori-penyakit.index') }}" class="flex items-center w-full md:w-1/3 mb-8">
                        <input type="text" name="search" placeholder="Cari Nama atau Kode ICD..." 
                            value="{{ request('search') }}"
                            class="search-input"
                            aria-label="Cari kategori penyakit">
                        ...
                        @if (request('search'))
                            <a href="{{ route('kategori-penyakit.index') }}" class="text-sm text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 font-semibold ml-3">Reset</a>
                        @endif
                    </form>
                    ...
                    <div class="overflow-x-auto">
                            ...
                            <table class="data-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left">Kode ICD</th>
                                ...
                                <tbody>
                                    @foreach ($penyakits as $index => $penyakit)
                                        <tr>
                                            <td class="whitespace-nowrap text-center">{{ $index + $penyakits->firstItem() }}</td>
                                            ...
                                            <td class="whitespace-nowrap text-center space-x-2">
                                                <a href="{{ route('kategori-penyakit.show', $penyakit) }}" class="action-link link-detail">
                                                    <i class="fas fa-eye"></i>Detail
                                                </a>

                                                <a href="{{ route('kategori-penyakit.edit', $penyakit) }}" class="action-link link-edit">
                                                    <i class="fas fa-edit"></i>Edit
                                                </a>

                                                <form action="{{ route('kategori-penyakit.destroy', $penyakit) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori {{ $penyakit->nama_penyakit }}? Aksi ini tidak dapat dibatalkan.');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @else
                            <p class="text-center text-gray-500 dark:text-gray-400 py-8">Belum ada data kategori penyakit yang tercatat.</p>
                        @endif
                        
                        <div class="pagination-container">
                             <div class="text-sm text-gray-700 dark:text-gray-400">
                                Showing {{ $penyakits->firstItem() }} to {{ $penyakits->lastItem() }} of {{ $penyakits->total() }} results
                            </div>
